...
class Layer_Input:
    # This is to make out Model Class easier to code, so we can refference the input layer in the forward and backward pass
    def forward(self, inputs, training):
        self.output = inputs

class Layer_Dropout:
    def __init__(self, rate):
        # rate is probability of neuron turning off, self.rate is probability of staying on
        self.rate = 1 - rate
...
class Activation_Softmax:
    def forward(self, inputs, training):
        self.inputs = inputs
        exp_values = np.exp(inputs - np.max(inputs, axis=1, keepdims=True)) # subtract the max to prevent overflow, the probability distrubution will be exactly the same
...
class Activation_GELU:
    def forward(self, inputs, training):
        self.inputs = inputs
        # Approximate GELU with tanh formulation
        self.output = 0.5 * inputs * (1.0 + np.tanh(np.sqrt(2 / np.pi) * (inputs + 0.044715 * np.power(inputs, 3))))

    def backward(self, dvalues):
        x = self.inputs
        tanh_out = np.tanh(np.sqrt(2/np.pi) * (x + 0.044715 * x**3))
        sech2 = 1 - tanh_out**2  # derivative of tanh

        # Derivative of GELU wrt input (approximation)
        self.dinputs = dvalues * (0.5 * (1.0 + tanh_out) + 0.5 * x * sech2 * np.sqrt(2/np.pi) * (1 + 3*0.044715 * x**2))
...
class Loss_CategoricalCrossentropy(Loss):
    def forward(self, y_pred, y_true):
        samples = len(y_pred)
        y_pred_clipped = np.clip(y_pred, 1e-7, 1-1e-7) # to prevent taking log of 0
    ...
    def backward(self, dvalues, y_true):
        ...
        # turn into one-hot encoded vectors
        if len(y_true.shape) == 1:
...
class  Activation_Softmax_Loss_CategoricalCrossentropy():
    ...
    def backward(self, dvalues, y_true):
        samples = len(dvalues)
        # turn out of one-hot encoded vectors
...
class Model:
    ...
    def finalise(self):
        ...
        for i in range(layer_count):
            ...
            # So on training only trainable layers are acessed
    ...
    def backward(self, output, y):
        if self.softmax_clasifier_output is not None:
            self.softmax_clasifier_output.backward(output, y)
